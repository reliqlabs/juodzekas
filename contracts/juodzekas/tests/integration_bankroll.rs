...
use cw_multi_test::{App, AppBuilder, AppResponse, BankKeeper, ContractWrapper, DistributionKeeper,
                    Executor, FailingModule, GovFailingModule, IbcFailingModule,
                    Stargate, StakeKeeper, WasmKeeper};
use juodzekas::msg::{DealerBalanceResponse, DealerResponse, ExecuteMsg, GameResponse, InstantiateMsg, QueryMsg};
...
impl Stargate for ZkMockStargate {
    fn execute_stargate<ExecC, QueryC>(
        &self, _api: &dyn cosmwasm_std::Api, _storage: &mut dyn cosmwasm_std::Storage,
        _router: &dyn cw_multi_test::CosmosRouter<ExecC = ExecC, QueryC = QueryC>,
        _block: &cosmwasm_std::BlockInfo, _sender: Addr, _type_url: String, _value: Binary,
    ) -> cosmwasm_std::StdResult<AppResponse>
    where ExecC: cosmwasm_std::CustomMsg + serde::de::DeserializeOwned + 'static,
          QueryC: cosmwasm_std::CustomQuery + serde::de::DeserializeOwned + 'static,
    { Ok(AppResponse::default()) }

    fn query_stargate(
        &self, _api: &dyn cosmwasm_std::Api, _storage: &dyn cosmwasm_std::Storage,
        _querier: &dyn cosmwasm_std::Querier, _block: &cosmwasm_std::BlockInfo,
        _path: String, _data: Binary,
    ) -> cosmwasm_std::StdResult<Binary> {
        let mut buf = Vec::new();
        ProofVerifyResponse { verified: true }.encode(&mut buf).unwrap();
        Ok(Binary::from(buf))
    }
    ...
    fn execute_any<ExecC, QueryC>(
        &self, _api: &dyn cosmwasm_std::Api, _storage: &mut dyn cosmwasm_std::Storage,
        _router: &dyn cw_multi_test::CosmosRouter<ExecC = ExecC, QueryC = QueryC>,
        _block: &cosmwasm_std::BlockInfo, _sender: Addr, _msg: AnyMsg,
    ) -> cosmwasm_std::StdResult<AppResponse>
    where ExecC: cosmwasm_std::CustomMsg + serde::de::DeserializeOwned + 'static,
          QueryC: cosmwasm_std::CustomQuery + serde::de::DeserializeOwned + 'static,
    { Ok(AppResponse::default()) }
...
struct SeededGame { seed: u64 }

impl SeededGame {
    fn new(seed: u64) -> Self { Self { seed } }

    fn dealer_shuffled_deck(&self) -> Vec<Binary> {
        (0..52).map(|i| Binary::from(format!("d_{}_{}", self.seed, i).as_bytes())).collect()
    }
    fn player_shuffled_deck(&self) -> Vec<Binary> {
        (0..52).map(|i| Binary::from(format!("p_{}_{}", self.seed, i).as_bytes())).collect()
    }
...
fn setup_with_bankroll(initial_bankroll: u128) -> TestEnv {
    ...
    let contract = app.instantiate_contract(
        code_id, dealer.clone(), &default_instantiate_msg(),
        &funds, "juodzekas", Some(dealer.to_string()),
    ).unwrap();
...
fn query_dealer_balance(env: &TestEnv) -> Uint128 {
    let resp: DealerBalanceResponse = env.app.wrap().query_wasm_smart(
        &env.contract, &QueryMsg::GetDealerBalance {},
    ).unwrap();
    resp.balance
}
...
fn query_game(env: &TestEnv, game_id: u64) -> GameResponse {
    env.app.wrap().query_wasm_smart(
        &env.contract, &QueryMsg::GetGame { game_id },
    ).unwrap()
}
...
fn create_and_deal(
    ...
    let join_resp = env.app.execute_contract(
...
fn reveal_card(env: &mut TestEnv, game: &SeededGame, game_id: u64, card_index: u32, card_value: u8) {
    env.app.execute_contract(
        env.player.clone(), env.contract.clone(),
        &ExecuteMsg::SubmitReveal {
            game_id, card_index,
            partial_decryption: game.player_partial(card_index),
            proof: Binary::from(b"p"), public_inputs: vec![],
        }, &[],
    ).unwrap();
...
fn test_blackjack_payout() {
    ...
    let g = query_game(&env, game_id);
    assert!(g.status.contains("Blackjack"), "Expected blackjack, got: {}", g.status);
...
fn test_split_full_game() {
    ...
    let g = query_game(&env, game_id);
    assert!(g.status.contains("Player"), "Expected player win on both hands, got: {}", g.status);
...
fn test_timeout_player() {
    ...
    let g = query_game(&env, game_id);
    assert!(g.status.contains("Dealer"), "Expected dealer wins on timeout, got: {}", g.status);
...
fn test_timeout_dealer() {
    ...
    let g = query_game(&env, game_id);
    assert!(g.status.contains("Player"), "Expected player wins on dealer timeout, got: {}", g.status);
...
fn test_partial_withdraw() {
    ...
    env.app.execute_contract(
        env.dealer.clone(), env.contract.clone(),
        &ExecuteMsg::WithdrawBankroll { amount: Some(Uint128::new(50_000)) }, &[],
    ).unwrap();
    ...
    let err = env.app.execute_contract(
        env.dealer.clone(), env.contract.clone(),
        &ExecuteMsg::WithdrawBankroll { amount: Some(Uint128::new(999_999)) }, &[],
    ).unwrap_err();
...
fn test_dealer_hits_soft_17() {
    ...
    assert!(g.status.contains("Dealer"), "Expected dealer win after hitting soft 17, got: {}", g.status);
}
...
fn test_player_bj_vs_dealer_bj() {
    ...
    assert!(g.status.contains("Push"), "Expected push for BJ vs BJ, got: {}", g.status);
...
fn test_authorization_wrong_sender() {
    ...
    // Fund stranger
    env.app.send_tokens(env.dealer.clone(), stranger.clone(), &[Coin::new(1_000_000u128, "utoken")]).unwrap();

    // Stranger can't CreateGame (not the dealer)
    let err = env.app.execute_contract(
        stranger.clone(), env.contract.clone(),
        &ExecuteMsg::CreateGame {
            public_key: Binary::from(b"dpk"),
            shuffled_deck: game.dealer_shuffled_deck(),
            proof: Binary::from(b"proof"), public_inputs: vec![],
    ...
    let err = env.app.execute_contract(
        stranger.clone(), env.contract.clone(),
        &ExecuteMsg::WithdrawBankroll { amount: Some(Uint128::new(1)) }, &[],
    ).unwrap_err();
    assert!(err.to_string().contains("Only the dealer"));
...
fn test_multiple_concurrent_games() {
    ...
    // Create game 1
    let resp1 = env.app.execute_contract(
        env.dealer.clone(), env.contract.clone(),
        &ExecuteMsg::CreateGame {
            public_key: Binary::from(b"dpk"),
    ...
    // Create game 2
    let resp2 = env.app.execute_contract(
        env.dealer.clone(), env.contract.clone(),
        &ExecuteMsg::CreateGame {
            public_key: Binary::from(b"dpk"),
            shuffled_deck: game2.dealer_shuffled_deck(),
    ...
    env.app.send_tokens(env.player.clone(), player2.clone(), &[Coin::new(100_000u128, "utoken")]).unwrap();
    ...
    env.app.execute_contract(
        player2.clone(), env.contract.clone(),
        &ExecuteMsg::JoinGame {
            bet: Uint128::new(bet),
    ...
    assert!(g2.status.contains("Player"), "Expected player win game 2, got: {}", g2.status);
...
fn test_deposit_bankroll() {
    let mut env = setup_with_bankroll(0);
    let stranger = MockApi::default().addr_make("stranger");
    env.app.send_tokens(env.dealer.clone(), stranger.clone(), &[Coin::new(100_000u128, "utoken")]).unwrap();
    ...
    // GetDealer query
    let resp: DealerResponse = env.app.wrap().query_wasm_smart(
        &env.contract, &QueryMsg::GetDealer {},
    ).unwrap();
...
fn test_cancel_game_success() {
    ...
    let err: Result<GameResponse, _> = env.app.wrap().query_wasm_smart(
        &env.contract, &QueryMsg::GetGame { game_id },
    );
    assert!(err.is_err());
}
...
fn test_cancel_game_already_joined() {
    ...
    assert!(err.to_string().contains("Can only cancel games waiting for a player"));
}
...
fn test_withdraw_blocked_during_active_game() {
    ...
    assert!(err.to_string().contains("Cannot withdraw while unsettled games exist"));
}
...
fn test_player_bust_dealer_wins() {
    ...
    let g = query_game(&env, game_id);
    assert!(g.status.contains("Dealer"), "Player bust means dealer wins, got: {}", g.status);
...
fn test_create_game_invalid_deck_length() {
    ...
    let short_deck: Vec<Binary> = (0..10).map(|i| Binary::from(format!("card_{i}").as_bytes())).collect();
...
fn test_instantiate_zero_denominator_rejected() {
    ...
    msg.blackjack_payout = PayoutRatio { numerator: 3, denominator: 0 };

    let err = app.instantiate_contract(
        code_id, dealer.clone(), &msg,
        &[], "juodzekas", Some(dealer.to_string()),
    ).unwrap_err();
...
fn test_timeout_payout_goes_to_player() {
    ...
    env.app.send_tokens(env.dealer.clone(), stranger.clone(), &[Coin::new(100u128, "utoken")]).unwrap();
    ...
    let player_balance_before = env.app.wrap()
    ...
    let player_balance_after = env.app.wrap()
        .query_balance(&env.player, "utoken").unwrap().amount;
    let stranger_balance = env.app.wrap()
        .query_balance(&stranger, "utoken").unwrap().amount;

    assert!(player_balance_after > player_balance_before, "Player should receive timeout payout");
    assert_eq!(stranger_balance, cosmwasm_std::Uint256::from(100u128), "Stranger should not receive funds");
}
...
fn test_split_bust_hand_stays_busted() {
    ...
    assert!(g.status.contains("Dealer"), "Hand 0 should be dealer win (bust), got: {}", g.status);
    assert!(g.status.contains("Player"), "Hand 1 should be player win, got: {}", g.status);
...
fn test_21_autostand_advances_hand_index_in_split() {
    ...
    let g = query_game(&env, game_id);
    assert!(g.status.contains("Player"), "Both hands should win vs bust dealer, got: {}", g.status);
}
...
fn test_join_game_rejects_excess_funds() {
    ...
    assert!(err.to_string().contains("Must send exact bet amount"),
        "Expected exact amount error, got: {}", err);
}
...
fn test_split_hand_21_not_blackjack() {
    ...
    assert!(g.status.contains("Push"), "Split hand 21 vs dealer 21 should push, got: {}", g.status);
    ...
    let bal = query_dealer_balance(&env);
    assert_eq!(bal, Uint128::new(100_000), "Push should return full bankroll to dealer");
}
...
fn test_stand_updates_timestamp() {
    ...
    assert!(err.to_string().contains("Timeout not reached"),
        "Timeout should not be reached yet, got: {}", err);
...
fn test_timeout_blame_during_reveal_targets_dealer() {
    ...
    let player_bal_before = env.app.wrap()
        .query_balance(&env.player, "utoken").unwrap().amount;
    ...
    let g = query_game(&env, game_id);
    assert!(g.status.contains("Player"), "Dealer lagged on reveal, player should win, got: {}", g.status);

    // Player should receive 2x bet
    let player_bal_after = env.app.wrap()
        .query_balance(&env.player, "utoken").unwrap().amount;
    let received = player_bal_after.checked_sub(player_bal_before).unwrap();
    assert_eq!(received, cosmwasm_std::Uint256::from(2000u128), "Player should get 2x bet");
}
...
fn test_timeout_blame_during_reveal_targets_player() {
    ...
    let g = query_game(&env, game_id);
    assert!(g.status.contains("Dealer"), "Player lagged on reveal, dealer should win, got: {}", g.status);
...
fn test_empty_partial_decryption_rejected() {
    ...
    assert!(err.to_string().contains("Partial decryption cannot be empty"),
        "Expected empty partial error, got: {}", err);
}
...
fn test_all_hands_busted_skips_dealer_reveal() {
    ...
    let g = query_game(&env, game_id);
    assert!(g.status.contains("Dealer"), "All hands busted → dealer wins, got: {}", g.status);

    // Dealer hand should still only have the upcard (no hole card revealed)
    assert_eq!(g.dealer_hand.len(), 1, "Dealer should only have upcard, no hole card reveal");
...
fn test_split_all_busted_skips_dealer_reveal() {
    ...
    let g = query_game(&env, game_id);
    assert!(g.status.contains("Dealer"), "All split hands busted → dealer wins, got: {}", g.status);

    // Dealer hand should only have upcard
    assert_eq!(g.dealer_hand.len(), 1, "Dealer should only have upcard after all-bust settle");
