use crate::elgamal::{Ciphertext, encrypt};
use crate::babyjubjub::{Point, Fr, Fq, is_y_negative};
...
pub fn shuffle<R: Rng>(
    ...
    let u_delta0: Vec<Bn254Fr> = deck.iter().enumerate().map(|(i, c)| {
        let y_full = c.c0.y;
        ...
        // Therefore: s = !is_neg
        if !is_neg { s_u0 += Bn254Fr::from(1u128 << i); }
        let delta = if is_neg { -y_full } else { y_full };
        Bn254Fr::from_le_bytes_mod_order(&delta.into_bigint().to_bytes_le())
    }).collect();

    let u_delta1: Vec<Bn254Fr> = deck.iter().enumerate().map(|(i, c)| {
        let y_full = c.c1.y;
        let is_neg = is_y_negative(y_full);
        if !is_neg { s_u1 += Bn254Fr::from(1u128 << i); }
        let delta = if is_neg { -y_full } else { y_full };
        Bn254Fr::from_le_bytes_mod_order(&delta.into_bigint().to_bytes_le())
    }).collect();

    let v_delta0: Vec<Bn254Fr> = re_encrypted_deck.iter().enumerate().map(|(i, c)| {
        let y_full = c.c0.y;
        let is_neg = is_y_negative(y_full);
        if !is_neg { s_v0 += Bn254Fr::from(1u128 << i); }
        let delta = if is_neg { -y_full } else { y_full };
        Bn254Fr::from_le_bytes_mod_order(&delta.into_bigint().to_bytes_le())
    }).collect();

    let v_delta1: Vec<Bn254Fr> = re_encrypted_deck.iter().enumerate().map(|(i, c)| {
        let y_full = c.c1.y;
        let is_neg = is_y_negative(y_full);
        if !is_neg { s_v1 += Bn254Fr::from(1u128 << i); }
        let delta = if is_neg { -y_full } else { y_full };
        Bn254Fr::from_le_bytes_mod_order(&delta.into_bigint().to_bytes_le())
    }).collect();

    let public_inputs = ShufflePublicInputs {
        pk: [Bn254Fr::from_le_bytes_mod_order(&aggregated_pk.x.into_bigint().to_bytes_le()),
             Bn254Fr::from_le_bytes_mod_order(&aggregated_pk.y.into_bigint().to_bytes_le())],
        ux0: ux0.iter().map(|f| Bn254Fr::from_le_bytes_mod_order(&f.into_bigint().to_bytes_le())).collect(),
        ux1: ux1.iter().map(|f| Bn254Fr::from_le_bytes_mod_order(&f.into_bigint().to_bytes_le())).collect(),
        vx0: vx0_prime.iter().map(|f| Bn254Fr::from_le_bytes_mod_order(&f.into_bigint().to_bytes_le())).collect(),
        vx1: vx1_prime.iter().map(|f| Bn254Fr::from_le_bytes_mod_order(&f.into_bigint().to_bytes_le())).collect(),
        s_u: [s_u0, s_u1],
        s_v: [s_v0, s_v1],
    };
